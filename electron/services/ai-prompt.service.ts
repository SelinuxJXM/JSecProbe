...
请直接返回回答文本，不要JSON格式。`,
  },
  {
    ...
    name: '核查命令智能推荐',
    description: '根据测评项和资产信息从命令库中推荐最相关的核查命令',
    variables: [
    ...
    builtinTemplate: `你是一名专业的等级保护测评师。请根据以下测评项信息和资产信息，从候选命令列表中推荐最相关的核查命令。
...
要求：
- 只从候选命令列表中选择，不得推荐列表之外的命令
- 推荐与测评项内容直接相关、且与资产信息（品牌/系统/设备类型）匹配的命令
- 优先推荐能直接验证测评项符合性的命令（如配置核查、策略查看、日志审计类）
- 最多推荐5条，按相关度从高到低排序
- 若没有合适的命令，返回空数组
...
要求：
- 优先从系统描述中提取有明确依据的资产；也可结合等保测评经验合理补充常见必备资产（如业务系统对应的服务器、数据库、管理终端等），但不得凭空编造明显不存在的设备
